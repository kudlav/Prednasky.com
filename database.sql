...
-- Tue Feb  5 11:25:21 2019
-- Model: Prednasky.com    Version: 1.11
...
CREATE TABLE IF NOT EXISTS `prednasky`.`user` (
  ...
  `right_group` TINYINT UNSIGNED NOT NULL COMMENT 'disabled, guest, student, teacher, admin',
...
CREATE TABLE IF NOT EXISTS `prednasky`.`video` (
  ...
  `record_time_begin` TIME NULL,
  `record_time_end` TIME NULL,
...
CREATE TABLE IF NOT EXISTS `prednasky`.`file` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `fk_file_user1_idx` (`user` ASC),
...
CREATE TABLE IF NOT EXISTS `prednasky`.`video_has_file` (
  ...
  `type` VARCHAR(45) NOT NULL COMMENT 'thumbnail, video, attachment',
...
CREATE TABLE IF NOT EXISTS `prednasky`.`video_relation` (
  ...
  PRIMARY KEY (`video_from`, `video_to`, `relation_type_id`),
